...
contract MeUPlatform is Ownable, ReentrancyGuard {
    MeUToken public token;
    uint256 public courseIdCounter;
    uint256 public registrationFee;
    uint256 public stakingFee = 1 * 10 ** 18; // 1 MEUT token
    ...
    constructor(MeUToken _token, uint256 _registrationFee) Ownable(msg.sender) {
        token = _token;
    ...
    function attestCourseCompletion(uint256 courseId, address student) public {
        ...
        // Reward student with 5 METU tokens
        token.mint(student, 5 * 10 ** 18); // 5 MEUT tokens
